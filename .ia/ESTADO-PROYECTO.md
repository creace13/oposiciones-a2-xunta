...
- 🟡 **Seguridad técnica básica y rendimiento (C02-07)**: implementado y subido a GitHub en `caa052f`: retirado el CDN público, la URL y la clave pública de Supabase, añadidas cabeceras de seguridad en Worker, protegidas metas locales frente a HTML inyectado, auditadas dependencias de producción con 0 vulnerabilidades y añadida `scripts/test-security-basics.js`. Pendiente: Cloudflare sigue sirviendo el HTML anterior y debe desplegarse desde panel o con token del titular.
...
- Desbloquear la publicación de `C02-07` en Cloudflare: el repositorio ya está en `caa052f`, pero producción aún sirve la versión anterior.
- Después ejecutar `C02-08`: mantenimiento normativo, inventario de vigencia, cadencia de revisión, alerta de cambios y retirada de preguntas afectadas.
- Con `C02-07` y `C02-08` cerrados y `C02-09` mantenido como bloqueo externo documentado, preparar `C02-10` como aceptación final de la versión estable local.
...
- **Publicación:** GitHub está actualizado a `caa052f`, pero Cloudflare Workers sigue sirviendo la versión anterior tras el push. Wrangler no puede desplegar desde esta sesión sin `CLOUDFLARE_API_TOKEN`. Requiere despliegue manual desde el panel o token temporal/controlado por el titular.
