...
| **C02-07** | P1 | Revisar seguridad, privacidad técnica, dependencias, cabeceras, errores y rendimiento de producción. | `HECHO` (Codex) | Cerrada en producción. Cloudflare aplica cabeceras desde `public/_headers`, mecanismo correcto para Workers Static Assets. | `scripts/test-security-basics.js` OK, `npm audit --omit=dev --audit-level=high` con 0 vulnerabilidades, `npm test` OK, Supabase CDN retirado, HTML público actualizado y cabeceras CSP/nosniff/referrer/x-frame/permissions/HSTS comprobadas con `curl -D -`. |
| **C02-08** | P1 | Establecer mantenimiento normativo: inventario de vigencia, cadencia de revisión, alerta de cambios y retirada de preguntas afectadas. | `HECHO` (Codex) | Cerrada con procedimiento y simulación automatizada. | `docs/MANTENIMIENTO-NORMATIVO.md`, `scripts/test-normative-maintenance.js`, `npm test` OK; simulación Ley 39/2015 localiza 153 preguntas a revisión. |
| **C02-09** | P1 | Obtener una revisión jurídica independiente del banco y registrar muestra, alcance, incidencias y resolución. | `BLOQUEADO` | Revisor humano jurídicamente competente designado por el titular. Se traslada como garantía externa futura y no bloquea la versión 1.2.0 estable local. | Dictamen externo identificado y todas las incidencias críticas resueltas. |
| **C02-10** | P2 | Ejecutar aceptación final, etiquetar la versión estable local, publicar y realizar smoke test posterior al despliegue. | `HECHO` (Codex) | C02-01 a C02-08 cerrados; C02-09 tratado como bloqueo externo documentado por falta de revisor jurídico independiente. | `docs/ACEPTACION-VERSION-1.2.0.md`, `scripts/test-release-acceptance.js`, `npm test` OK, commit `4c85a61`, producción HTTP 200 con `v21-stable-local-20260720` y cabeceras de seguridad. |

## Orden recomendado mientras el titular está ausente

No quedan tareas ejecutables abiertas para la versión 1.2.0 estable local. `C02-09` permanece como garantía jurídica externa futura si el titular designa revisor.
